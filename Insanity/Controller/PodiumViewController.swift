...
class PodiumViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        dataBrain.recupPodiumMaxValues()
        defaultPodiumValue()
    }
    ...
    func updatePodium(sportRow: Int) {
        ...
        let currentUserIndex = dataBrain.dataPodium.indices.filter { dataBrain.dataPodium[$0].userID == currentUserID}

        var commentScore = String(format: "%.0f", dataBrain.dataPodium[currentUserIndex[0]].max[sportRow])
        var commentLabel = K.podium.notOnPodium
   
        let dataPodiumFiltered = dataBrain.dataPodium.filter { $0.max.isEmpty == false }
        ...
        case 2:
            ...
            let index = dataBrain.dataPodium.indices.filter { dataBrain.dataPodium[$0].max == maxScore!.max }
            topOneImage.image = UIImage(named: dataBrain.dataPodium[index[0]].avatar)
            topOneLabel.text = String(format: "%.0f", dataBrain.dataPodium[index[0]].max[sportRow])
            topOnePseudoLabel.text = dataBrain.dataPodium[index[0]].pseudo
            if dataBrain.dataPodium[index[0]].userID == currentUserID {
            ...
            if index.count == 2 {
                topTwoImage.image = UIImage(named: dataBrain.dataPodium[index[1]].avatar)
                topTwoLabel.text = String(format: "%.0f", dataBrain.dataPodium[index[1]].max[sportRow])
                topTwoPseudoLabel.text = dataBrain.dataPodium[index[1]].pseudo
                if dataBrain.dataPodium[index[1]].userID == currentUserID {
                    commentLabel = K.podium.second
                    commentScore = ""
                }
            } else {
                let secondScore = dataPodiumFiltered.filter { $0.max != maxScore!.max }
                let indexSecond = dataBrain.dataPodium.indices.filter { dataBrain.dataPodium[$0].max == secondScore[0].max }
                topTwoImage.image = UIImage(named: dataBrain.dataPodium[indexSecond[0]].avatar)
                topTwoLabel.text = String(format: "%.0f", dataBrain.dataPodium[indexSecond[0]].max[sportRow])
                topTwoPseudoLabel.text = dataBrain.dataPodium[indexSecond[0]].pseudo
                if dataBrain.dataPodium[indexSecond[0]].userID == currentUserID {
                    commentLabel = K.podium.second
                    commentScore = ""
                }
            }
        ...
        case 3...4:
            ...
            if index.count == 3 || index.count == 4 {
                ...
                topTwoLabel.text = String(format: "%.0f", dataBrain.dataPodium[index[1]].max[sportRow])
                topTwoPseudoLabel.text = dataBrain.dataPodium[index[1]].pseudo
                topThreeImage.image = UIImage(named: dataBrain.dataPodium[index[2]].avatar)
                topThreeLabel.text = String(format: "%.0f", dataBrain.dataPodium[index[2]].max[sportRow])
                topThreePseudoLabel.text = dataBrain.dataPodium[index[2]].pseudo
                if dataBrain.dataPodium[index[1]].userID == currentUserID {
                    commentLabel = K.podium.second
                    commentScore = ""
                } else if dataBrain.dataPodium[index[2]].userID == currentUserID {
                    commentLabel = K.podium.third
                    commentScore = ""
                } else {
                    if index.count == 4 && dataBrain.dataPodium[index[3]].userID == currentUserID {
                        commentLabel = K.podium.notOnPodium
                        commentScore = String(format: "%.0f", dataBrain.dataPodium[index[3]].max[sportRow])
                    }
                }
                ...
                let indexSecond = dataBrain.dataPodium.indices.filter { dataBrain.dataPodium[$0].max == SecondMaxScore!.max }
                topTwoImage.image = UIImage(named: dataBrain.dataPodium[indexSecond[0]].avatar)
                topTwoLabel.text = String(format: "%.0f", dataBrain.dataPodium[indexSecond[0]].max[sportRow])
                topTwoPseudoLabel.text = dataBrain.dataPodium[indexSecond[0]].pseudo
                if dataBrain.dataPodium[indexSecond[0]].userID == currentUserID {
                ...
                if indexSecond.count == 2 || indexSecond.count == 3 {
                    ...
                    topThreeImage.image = UIImage(named: dataBrain.dataPodium[indexSecond[1]].avatar)
                    topThreeLabel.text = String(format: "%.0f", dataBrain.dataPodium[indexSecond[1]].max[sportRow])
                    topThreePseudoLabel.text = dataBrain.dataPodium[indexSecond[1]].pseudo
                    if dataBrain.dataPodium[indexSecond[1]].userID == currentUserID {
                        commentLabel = K.podium.third
                        commentScore = ""
                    }
                    if indexSecond.count == 3 && dataBrain.dataPodium[indexSecond[2]].userID == currentUserID{
                        commentLabel = K.podium.notOnPodium
                        commentScore = String(format: "%.0f", dataBrain.dataPodium[indexSecond[2]].max[sportRow])
                    }
                } else {
                    ...
                    let indexThird = dataBrain.dataPodium.indices.filter { dataBrain.dataPodium[$0].max == thirdMaxScore!.max }
                    topThreeImage.image = UIImage(named: dataBrain.dataPodium[indexThird[0]].avatar)
                    topThreeLabel.text = String(format: "%.0f", dataBrain.dataPodium[indexThird[0]].max[sportRow])
                    topThreePseudoLabel.text = dataBrain.dataPodium[indexThird[0]].pseudo
                    if dataBrain.dataPodium[indexThird[0]].userID == currentUserID {
                          commentLabel = K.podium.third
                        commentScore = ""
                    }
                }
